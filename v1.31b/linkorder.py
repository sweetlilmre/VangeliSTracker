...
# our filename -> the unit name TP puts in the map. Units whose name differs from
# their file's stem, plus the two that emit no segment at all.
UNITNAME = {
    'MODCOMMA': 'MODCOMMANDS', 'SONGELEM': 'SONGELEMENTS',
    'SOUNDBLA': 'SOUNDBLASTER', 'SOUNDDEV': 'SOUNDDEVICES',
    'MODLOADE': 'MODLOADER', 'UNKLOADE': 'UNKLOADER',
    'VTMAIN': 'DEMOVT',
}
# no segment of their own in EITHER image: declarations only. Their position in the
# order is therefore unobservable and they are excluded from the diff.
NOSEG = {'VTGLOBAL', 'VTDRIVER'}

# The original's order, read off the segment ADDRESSES in docs/00-map.md -- the one
# thing about the original link that is directly measurable.
ORIG_ORDER = [
    'DEMOVT', 'VTSILENC', 'DEVGUS', 'VTCMD', 'FILEUTIL', 'CMDLINE', 'VTCFG',
    'PLAYMOD', 'MODCOMMANDS', 'VTCTRL', 'SONGUNIT', 'FILTERS', 'MODLOADER',
    'ASCIIZ', 'UNKLOADER', 'VTNOTES', 'SONGELEMENTS', 'GUS', 'HEAPS', 'VTDOSMEM',
    'VTDOSRSZ', 'OBJECTS', 'VTSHELL', 'DEVSB', 'SOUNDBLASTER', 'SOUNDDEVICES',
    'HARDWARE', 'VTRESID', 'DOS', 'SYSTEM',
]
...
def check_constraints(g):
    ...
    pos = {n: i for i, n in enumerate(reversed(ORIG_ORDER))}   # finish index
    bad = []
...
def main():
    ...
    for i in range(max(len(ours), len(ORIG_ORDER))):
        a = ours[i] if i < len(ours) else ''
        b = ORIG_ORDER[i] if i < len(ORIG_ORDER) else ''
        print("%-4d %-16s %-16s %s" % (i, a, b, '' if a == b else '  <-- differs'))
    same = sum(1 for a, b in zip(ours, ORIG_ORDER) if a == b)
    print("\n%d of %d positions agree" % (same, len(ORIG_ORDER)))
