...
Everything used to sit under `D:\source\psycho`. It is now its own repository:
    ...
    D:\source\VangeliSTracker\        <- the git root; ROOT in every script
      ...
      v1.31b/                         THE RECONSTRUCTION -- our work
        ...
        ref/vt1.31b.bin               the measurement target. UNTRACKED, see its README.
...
**THIS WORK IS NOW TRACKED, AND IT MOVED.** It used to live in `D:\source\psycho\demovt` and be excluded from source control pending a licence check. It is now its own repository at **`D:\source\VangeliSTracker`** — the user's decision — and the source, the docs and the tools are committed. Two things are still deliberately NOT committed, both in the root `.gitignore`:

* **`v1.31b/ref/vt1.31b.bin`** — the reference image. A third-party binary lifted out of someone else's release; the tree carries it for convenience and git does not. `v1.31b/ref/README.md` says how to recreate it, and that README *is* tracked.
* **`build/`** — DOSBox's scratch area. Everything in it is regenerated by `build.py`.

`v1.39b/`, the VangeliSTracker 1.39b source release, IS tracked: it is the reference source this whole reconstruction leans on, and losing it would cost more than keeping it.
...
The shipped file is LZEXE 0.91 packed. Unpacking is already done and the output is in place at **`v1.31b/ref/vt1.31b.bin`** — untracked, so a fresh clone has to make its own:
