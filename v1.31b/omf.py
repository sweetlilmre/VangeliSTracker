...
"""Read an Intel OMF .OBJ and report which bytes of its code are FIXUPS.
    ...
    python v1.31b/omf.py build/SOUNDDEV.OBJ

WHY THIS EXISTS. verify.py's whole model of an unresolved reference is "the
...
def main(argv):
    if not argv:
        print("usage: python v1.31b/omf.py <file.OBJ> [-v]")
        return 2
    code, fixups = code_and_fixups(argv[0])
    print("%s: %d bytes of code, %d fixup byte(s) in %d field(s)"
          % (argv[0], len(code), len(fixups), sum(1 for f in fixups if f - 1 not in fixups)))
    if "-v" in argv:
        runs = []
        for f in sorted(fixups):
            if runs and f == runs[-1][1]:
                runs[-1][1] = f + 1
            else:
                runs.append([f, f + 1])
        for lo, hi in runs:
            print("  +%04x  %d byte(s)  %s" % (lo, hi - lo, code[lo:hi].hex()))
    return 0
