...
MIXER = r'''
  { ==================================================================
    ...
    guard byte, for speed rather than for hiding.

    [re] That guard byte is at CS:$104b.
...
@@ChainOld:
  ...
  { ==================================================================
    ...
    stream, after the poll's guard, the filter chains' four sample
    immediates, and @@Flag0e74.

    [re] The poll's guard is at CS:$104b.
...
def main():
    ...
    w("")
    w("  THE GAIN LADDER: 256 unrolled scale-and-add steps, one per volume")
    w("  level, so that scaling a sample costs a call and an add rather")
    w("  than a multiply. On an 8086 a multiply is over a hundred cycles")
    w("  and this runs once per sample per channel, so the table is the")
    w("  difference between playing and not.")
    w("")
    w("  THE PATCHER IS WHAT MAKES IT USABLE. Each cell ends in a jump")
    w("  whose target is written at run time, so the mixer enters the")
    w("  ladder at the cell for the volume it wants and leaves where it")
    w("  was told to. The cells are the table AND the code.")
    w("")
    w("  [re] 1a17:0746..1a17:0b29, decoded from the image not typed.")
    w("")
    w("  [re] THE WHOLE REGION IS ONE PROCEDURE, and it has to be. The patcher writes")
    ...
    w("  table entry lies.")
    w("")
    w("  [re] THAT IS ALSO WHY THIS FILE HOLDS THE PROCEDURE AND NOT JUST THE DATA.")
    ...
    w("")
    w("  [re] THE MIXER AT 1a17:0b2a WILL HAVE TO BE EMITTED FROM HERE TOO, and not")
    ...
    w("")
    w("  [re] NO JMP OVER THE DATA, AND THAT WAS A THREE-BYTE ERROR. This procedure")
