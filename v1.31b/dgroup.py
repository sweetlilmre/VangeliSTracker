...
def orig_image():
    ...
    # DGROUP is segment 1caa and the load base is 1000; everything after it in the
    # file is initialised DGROUP, because it is the last segment in the image.
    return p[hdr + (0x1caa - 0x1000) * 16:]
...
def our_image():
    ...
    m = re.search(r'^\s*([0-9A-F]+)H\s+[0-9A-F]+H\s+[0-9A-F]+H\s+DATA\s+DATA\s*$',
                  mp, re.M)
    if not m:
        raise SystemExit("no DATA line in build/VTMAIN.MAP -- build with --sw=/GD")
    start = int(m.group(1), 16)
    p = (ROOT / 'build' / 'VTMAIN.EXE').read_bytes()
    hdr = struct.unpack_from('<H', p, 8)[0] * 16
    return p[hdr + start:]
...
def main():
    ...
    orig, ours = orig_image(), our_image()
