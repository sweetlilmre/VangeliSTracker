"""Per-block verification for segment 193a, because the prefix cannot measure it.

`verify.py`'s prefix stops at the first byte that disagrees, and while 193a is
...
target is in the right place.
    ...
    python v1.31b/block193a.py
    python v1.31b/block193a.py GetInstrument
"""
...
BLOCKS = [
    ("Names x4",            0x0000, 0x0014, True),
    ("SetVars",             0x0014, 0x002b, True),
    ("DevPoll",             0x002b, 0x0030, True),
    ("DMAIrq",              0x0030, 0x007f, True),
    ("DMASBGetRealFreq",    0x007f, 0x013c, True),
    ("DMASBProGetRealFreq", 0x013c, 0x0206, True),
    ("DMASBCalcTimerData",  0x0206, 0x02c2, True),
    ("SbMonoDetect",        0x02c2, 0x02e7, True),
    ("SbStereoDetect",      0x02e7, 0x031f, True),
    ("DMASBInit",           0x031f, 0x03c5, True),
    ("DMASBMonoInit",       0x03c5, 0x03e1, True),
    ("DMASBSterInit",       0x03e1, 0x03fd, True),
    ("DMASBMixInit",        0x03fd, 0x0419, True),
    ("DMASBMix2Init",       0x0419, 0x0435, True),
    ("DMASBChgHz",          0x0435, 0x0442, True),
    ("DMASBEnd",            0x0442, 0x048a, True),
    ("init section",        0x048a, 0x0651, True),
]
...
def main(argv):
    ...
    tpu_path = verify.BUILD / "DEVSB.TPU"
    if not tpu_path.exists():
        print("no DEVSB.TPU -- run v1.31b/build.py first")
        return 1
    ...
    print("segment 193a, unit code located at .TPU offset 0x%04x" % at)
    print()
