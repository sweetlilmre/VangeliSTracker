...
r"""Pack an MZ executable the way LZEXE 0.91 did, and compare against the original.
    ...
    python v1.31b/lzpack.py                    pack build/VTMAIN.EXE, compare, report
    python v1.31b/lzpack.py --out FILE         ...and keep the packed result
    python v1.31b/lzpack.py --tokens FILE      dump any LZ91 file's token stream
    python v1.31b/lzpack.py --exe FILE         pack something else
    python v1.31b/lzpack.py --selftest         re-compress every LZ91 file found,
                                               to show the encoder is not fitted

THE LAST GATE. `unlzexe.py` recovered the load image from `NEUROSIS.008` and this
tree reproduces that image byte for byte. What it could not reach was the PACKED
file the demo actually ships, because LZEXE is a 1989 third-party packer that is
not in this tree or the DOSBox image. This is the packer written out instead, so
the comparison runs end to end: our Pascal source -> TPC -> our EXE -> packed ->
against `NEUROSIS.008` itself.

WHERE THE ORIGINAL PACKED FILE COMES FROM. It is NOT tracked here -- only the
unpacked image is. Point `VT_PACKED` at a copy of `NEUROSIS.008`, or pass
`--packed FILE`. The sibling psycho checkout holds one at `bin/NEUROSIS.008`.
It is needed for two things: the comparison, and the 344-byte decompressor stub,
which is LZEXE's own machine code and is copied verbatim rather than invented.
...
them:
    ...
    the SEGMENT-STEP position     one token    at output offset 0xA001. It
                                               instructs the stub to advance
                                               its destination segment, so it
                                               is a property of the stub and
                                               the image length -- both of
                                               which we reproduce -- but the
                                               rule that puts it at 0xA001
                                               rather than a round boundary is
                                               NOT understood. See the note.
...
THE SEGMENT-STEP PUZZLE, left open honestly. The token sits at output offset
40,961 = 0xA001, and nothing obvious lands there: the destination window is
0xFFF0 bytes, the image is only 0xD710, and a forward write from offset zero
would never need a step at all. A destination starting at offset 0x5FFF would
wrap exactly there, which is a hypothesis rather than a reading. Settling it
means disassembling the stub, which is 344 bytes and has not been done.
"""
import os
import struct
import sys
...
SEGSTEP_AT = 0xA001       # output offset of the one segment-step token
CONTROL = (0x00, 0xF0)    # the lo/hi the original uses for segstep and eos

COSTS = {"lit": 9, "short": 12, "long": 18, "ext": 26}


def packed_path(arg=None):
    """The original packed file: --packed, then VT_PACKED, then a clear refusal."""
    p = arg or os.environ.get("VT_PACKED")
    if not p:
        raise SystemExit(
            "no original packed file.\n"
            "It is NOT tracked here -- only the unpacked image in ref/ is.\n"
            "Point VT_PACKED at a copy of NEUROSIS.008, or pass --packed FILE.\n"
            "It supplies both the comparison target and LZEXE's 344-byte stub.")
    q = pathlib.Path(p)
    if not q.is_file():
...
def tokenize(img, window=WINDOW, segstep_at=SEGSTEP_AT):
    ...
    p = 0
    while p < n - 1:
        if segstep_at is not None and p == segstep_at:
        ...
        while j >= 0 and lst[j] >= floor:
            ...
            if k > best_len:              # strictly greater: ties keep NEAREST
                ...
                if best_len == limit:
                    break
        kind = encoding(best_len, best_dist)
        if kind is None:
...
def compress(img, window=WINDOW, segstep_at=SEGSTEP_AT):
    toks = tokenize(img, window, segstep_at)
    w = BitWriter()
...
def selftest(paths):
    ...
    The segment-step positions are TAKEN FROM each original, because the rule
    that places them is not understood (see the module header). Everything else
    -- every match, distance, length and encoding choice -- is ours.
    """
    ...
    for path in paths:
        ...
        steps, pos = [], 0
        for k, a, _b in theirs:
            if k == "segstep":
                steps.append(pos)
            pos += 1 if k == "lit" else (a if k in ("short", "long", "ext") else 0)

        ours = tokenize(image, segstep_at=None)
        merged, pos, si = [], 0, 0
        for t in ours:
            while si < len(steps) and pos == steps[si]:
                merged.append(("segstep", 0, 0))
                si += 1
            merged.append(t)
            pos += 1 if t[0] == "lit" else (t[1] if t[0] in ("short", "long", "ext") else 0)
        # tokenize() already ends the stream with eos -- do NOT add a second one
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if a == "--out":
            ...
            while i < len(argv) and not argv[i].startswith("-"):
                ...
                i += 1
            if not rest:
                rest = find_lz91([ROOT / "DEMOVT15", ROOT / "v1.39b"])
                extra = os.environ.get("VT_PACKED")
                if extra and pathlib.Path(extra).is_file():
                    rest.append(pathlib.Path(extra))
                if not rest:
                    raise SystemExit("no LZ91 files found to test against. Pass some: --selftest FILE [FILE ...]")
    ...
    exepath = pathlib.Path(exe or (ROOT / "build" / "VTMAIN.EXE"))
    if not exepath.is_file():
        raise SystemExit("no EXE to pack: %s\n"
                         "Build first: python kit/tools/pascal/build.py build.toml --sw=/GS"
                         % exepath)
